...
$clearNotificationsQuery = "DELETE FROM `Notifications`";
$pdo->exec($clearNotificationsQuery);
            ...
            <div class="row  mt-5 mb-5">
                <div class="aboutus col-lg-8 mx-auto">
                    <div class="img">
                        ...
                        <form method="post" action="Delete_candidature.php">
                    ...
                    if (isset($_POST['jobTitle']) && $_POST['jobTitle'] !== '') {
                        ...
                        $stmt = $pdo->prepare("SELECT i.InscriptionID, i.ApplicationDate, i.Status, o.JobTitle, e.CompanyName, e.Email, e.PhoneNumber FROM inscription i
                            ...
                            AND o.JobTitle = :jobTitle AND i.Status IS NOT = 'En attente'");
                        ...
                        if ($stmt->rowCount() > 0) {
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                if (isset($status) && $status == "En attente") {
                                    ...
                                    echo '</div>';
                                } elseif (isset($status) && $status == 'Accepté') {
                                    echo '<div class="alert alert-success " role="alert">';
                                    echo '<p class="p">Etat : ' . $status . '</p>';
                                    echo '</div>';
                                } elseif (isset($status) && $status == 'Rejeté') {
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                          <!-- Dropdown menu items -->
                          <li><a class="dropdown-item" href="Delete_candidature.php?inscriptionID=' . $row["InscriptionID"] . '">Retirer la candidature</a></li>
                        </ul>
                      </div>';
                                echo '</div>';
                            }
                        ...
                        $stmt = $pdo->prepare("SELECT i.InscriptionID, i.ApplicationDate, i.Status, o.JobTitle, e.CompanyName, e.Email, e.PhoneNumber
                                                        FROM inscription i
                                                        INNER JOIN offres o ON i.JobID = o.JobID
                                                        INNER JOIN employeur e ON o.EmployeurID = e.EmployeurID
                                                        WHERE i.CandidateID = :candidateID AND i.Status != 'En attente'");
